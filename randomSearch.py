# This script is a mod of the original ptb-lm script that performs a Random Search, and implements early stopping
...
parser.add_argument('--dp_keep_prob', type=float, default=None,
...
parser.add_argument('--nb_searches', type=int, default=1)  # number of performed random searches
...
for r in range(args.nb_searches):
    ...
    parser.add_argument('--best_val', type=float, default=None)
    parser.add_argument('--early_stopped', type=str, default='No')
    ...
    print("\n########## Random search ", r+1, "/", args.nb_searches, "###################")
    
    # we look for (GRU):
    #GRU SGD_LR_SCHEDULE 10.0lr - 2layers1500, 20batchSize, embSize200, 0.35keepProb, 35seqLen, 40epochs
    
    if(not args.seq_len):
        tab = [32, 35, 37]
        args.seq_len = random.choice(tab)
        print("random seq_len: ", args.seq_len)
    ...
    if(not args.batch_size):
        if(args.model == 'TRANSFORMER'):
        	...
        	tab = [10, 20, 25]
        args.batch_size = random.choice(tab)
        print("random batch_size: ", args.batch_size)
        
    if(not args.optimizer):
        tab = ['SGD_LR_SCHEDULE', 'ADAM']
        args.optimizer = random.choice(tab)
        print("random optimizer: ", args.optimizer)
        
    if(not args.initial_lr):
        if(args.optimizer == 'ADAM'):
            tab = [0.0001, 0.0005, 0.001]
        else:
        	tab = [10, 15, 5]
        args.initial_lr = random.choice(tab)
        print("random initial_lr: ", args.initial_lr)
        
    if(not args.n_heads):
        tab = [8, 16, 32, 64]
        args.n_heads = random.choice(tab)
        print("random n_heads: ", args.n_heads)
        
    if(not args.hidden_size): # multiples of n_heads for TRANSFORMER
        if(args.model == 'TRANSFORMER'):
            tab = [64, 128, 256, 512, 1024] # all multiples of 64, 32, 16, 8
        else:
        	tab = [1500, 1600, 1400]
        args.hidden_size = random.choice(tab)
        print("random hidden_size: ", args.hidden_size)
    ...
    if(not args.num_layers):
        if(args.model == 'TRANSFORMER'):
        	...
        	tab = [2, 3]
        args.num_layers = random.choice(tab)
        print("random num_layers: ", args.num_layers)
        
    if(not args.emb_size):
        tab = [200, 250, 500]
        args.emb_size = random.choice(tab)
        print("random emb_size: ", args.emb_size)
        
    if(not args.num_epochs):
        tab = [40, 100]
        args.num_epochs = random.choice(tab)
        print("random num_epochs: ", args.num_epochs)
        
    if(not args.dp_keep_prob):
        tab = [0.3, 0.35, 0.4, 0.9]
        args.dp_keep_prob = random.choice(tab)
        print("random dp_keep_prob: ", args.dp_keep_prob)
        
    if(not args.patience):
        tab = [3, 5]
        args.patience = random.choice(tab)
        if(args.patience > args.num_epochs):
            args.patience = args.num_epochs
        print("random patience: ", args.patience)
    ...
    if args.model == 'RNN':
        ...
        if args.debug:  # use a very small model
            ...
            model = TRANSFORMER(vocab_size=vocab_size, n_units=args.hidden_size, 
                                n_blocks=args.num_layers, n_heads=args.n_heads, dropout=1.-args.dp_keep_prob) 
    ...
    def run_epoch(model, data, is_train=False, lr=1.0):
        ...
        # for step, (x, y) in enumerate(ptb_iterator(data, model.batch_size, model.seq_len)):
            ...
            loss = loss_fn(outputs.contiguous().to(device).view(-1, model.vocab_size), tt)
            ...
            if is_train:  # Only update parameters if training 
                ...
                pbar.set_description('step: '+ str(step) + ' - loss: '+ str(costs) + ' - wps: '+ str(iters * model.batch_size / (time.time() - start_time)))
    ...
    for epoch in range(num_epochs):
        ...
        if args.save_best:
            with open (os.path.join(args.save_dir, 'log.txt'), 'a') as f_:
                f_.write(log_str+ '\n')
        args.best_val = best_val_so_far
        waiting = waiting + 1
        if(waiting > args.patience):
            args.early_stopped = 'Yes'
            break
...
output.drop(columns=['code_file', 'data', 'debug', 'evaluate', 'nb_searches', 'save_best', 'save_dir', 'seed'],inplace=True, axis=1)
so = output.sort_values(by="best_val",ascending=True)
display(so)
so.to_csv('lastresults.csv', sep='\t')
